...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  try {
    ...
    const validTools = toolDefinitions.filter(tool => {
      ...
      if (!isValid) {
        console.warn(`警告: 跳過無效的工具定義: ${tool?.name || '未命名'}`);
      }
    ...
    const toolsByCategory = {
      '商家管理': validTools.filter(t => t.name.startsWith('business')),
      '客戶管理': validTools.filter(t => t.name.startsWith('customer')),
      '預約管理': validTools.filter(t => t.name.startsWith('booking')),
      '員工管理': validTools.filter(t => t.name.startsWith('staff')),
      '服務管理': validTools.filter(t => t.name.startsWith('service')),
      '其他工具': validTools.filter(t => 
        !t.name.startsWith('business') && 
        !t.name.startsWith('customer') && 
        !t.name.startsWith('booking') && 
        !t.name.startsWith('staff') && 
...
(async () => {
  try {
    ...
    // 載入所有工具定義
    console.log('正在載入工具定義...');
    await loadToolDefinitions();
    
    // 記錄所有註冊的工具
    console.log(`已載入 ${toolDefinitions.length} 個工具：`);
    ...
    toolDefinitions.forEach(tool => {
      ...
      // 檢查工具定義是否完整
      if (!tool.inputSchema) {
        console.warn(`  警告: ${tool.name} 缺少輸入參數結構(inputSchema)`);
        isIncomplete = true;
      } else if (!tool.inputSchema.properties) {
        console.warn(`  警告: ${tool.name} 的輸入參數結構缺少 properties 定義`);
        isIncomplete = true;
      } else if (!tool.inputSchema.required || tool.inputSchema.required.length === 0) {
        console.warn(`  警告: ${tool.name} 的輸入參數結構缺少 required 欄位定義`);
    ...
    if (incomplete.length > 0) {
      console.warn(`警告: 有 ${incomplete.length} 個工具定義不完整: ${incomplete.join(', ')}`);
      console.warn('建議執行 "npm run generate-tools" 檢查並更新工具定義');
    }
