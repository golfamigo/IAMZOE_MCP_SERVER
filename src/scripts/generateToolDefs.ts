...
const toolDefinitionsFile = path.join(__dirname, '..', 'tools', 'toolDefinitions.ts');
    ...
    try {
      ...
      if (!toolDefinitionsContent.includes('generatedToolDefinitions.forEach')) {
        ...
        if (loadToolDefinitionsMatch) {
          ...
          const updatedFunc = loadToolDefinitionsFunc.replace(
