...
export interface AssignMembershipLevelResult {
  success: boolean;
}
 ...
 * @param params 指派參數
 */
export const assignMembershipLevelImpl = async (params: AssignMembershipLevelParams): Promise<AssignMembershipLevelResult> => {
  // 驗證輸入參數
  ...
  try {
    ...
    // 然後建立新的會員等級關係
    await neo4jClient.runQuery(cypher, parameters);
