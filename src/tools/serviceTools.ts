 ...
 * @returns 服務詳細資訊列表
 */
export const getServicesByBusinessIdImpl = async (params: { business_id: string }): Promise<BookableItem[]> => {
  const { business_id } = params;

  const result = await neo4jClient.runQuery(
    `MATCH (bi:BookableItem)-[:BELONGS_TO]->(b:Business {business_id: $business_id})
     RETURN bi`,
    { business_id }
  );
...
export const getServicesByBusinessId = createToolDefinition(
  'getServicesByBusinessId',
  '根據商家 ID 獲取服務項目詳細資訊',
  getServicesByBusinessIdSchema,
  getServicesByBusinessIdImpl
);
...
export const getAvailableSlotsImpl = async (params: GetAvailableSlotsParams): Promise<TimeSlot[]> => {
  ...
  let services: BookableItem[] = [];
  if (business_id) {
    // 根據 business_id 獲取所有服務
    services = await getServicesByBusinessIdImpl({ business_id });
  } else if (bookable_item_id) {
    // 根據 bookable_item_id 獲取服務
    const service = await getServiceImpl({ bookable_item_id });
    if (service) {
      services = [service];
    } else {
    ...
    throwInvalidParam('必須提供 bookable_item_id 或 business_id');
  }
  ...
  for (const service of services) {
    const { 
      bookable_item_id,
      business_id,
      bookable_item_duration: duration,
      bookable_item_max_capacity: maxCapacity,
      is_active: isActive
    } = service;
