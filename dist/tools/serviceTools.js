...
exports.serviceTools = exports.getAvailableSlots = exports.getService = exports.createService = exports.getAvailableSlotsImpl = exports.getServicesByBusinessId = exports.getServicesByBusinessIdImpl = exports.getServiceImpl = exports.createServiceImpl = void 0;
/**
 * 服務管理工具
...
/**
 * 根據 business_id 獲取服務資訊
 * @param params 查詢參數
 * @returns 服務詳細資訊列表
 */
const getServicesByBusinessIdImpl = async (params) => {
    const { business_id } = params;
    const result = await db_1.neo4jClient.runQuery(`MATCH (bi:BookableItem)-[:BELONGS_TO]->(b:Business {business_id: $business_id})
     RETURN bi`, { business_id });
    if (result.records.length === 0) {
        return []; // 如果沒有找到服務，返回空陣列
    }
    return result.records.map(record => record.get('bi').properties);
};
exports.getServicesByBusinessIdImpl = getServicesByBusinessIdImpl;
// 建立 getServicesByBusinessId 的工具定義
const getServicesByBusinessIdSchema = {
    type: 'object',
    properties: {
        business_id: { type: 'string', description: '商家 ID' }
    },
...
exports.getServicesByBusinessId = (0, toolRegistration_1.createToolDefinition)('getServicesByBusinessId', '根據商家 ID 獲取服務項目詳細資訊', getServicesByBusinessIdSchema, exports.getServicesByBusinessIdImpl);
/**
 * 獲取可用時間段
 * @param params 查詢參數
 * @returns 可用時間段列表
...
const getAvailableSlotsImpl = async (params) => {
    ...
    const { bookable_item_id, business_id, start_date, end_date } = params;
    let services = [];
    if (business_id) {
        // 根據 business_id 獲取所有服務
        services = await (0, exports.getServicesByBusinessIdImpl)({ business_id });
    }
    else if (bookable_item_id) {
        // 根據 bookable_item_id 獲取服務
        const service = await (0, exports.getServiceImpl)({ bookable_item_id });
        if (service) {
            services = [service];
        }
        else {
        ...
        (0, errorHandling_1.throwInvalidParam)('必須提供 bookable_item_id 或 business_id');
    }
    ...
    for (const service of services) {
        const { bookable_item_id, business_id, bookable_item_duration: duration, bookable_item_max_capacity: maxCapacity, is_active: isActive } = service;
        if (!isActive) {
       ...
       ORDER BY bh.day_of_week`, { business_id });
       ...
       RETURN s.staff_member_id as staff_id, sa.day_of_week as day_of_week, 
              sa.start_time as start_time, sa.end_time as end_time`, { business_id, bookable_item_id });
...
exports.serviceTools = {
    ...
    getServicesByBusinessId: exports.getServicesByBusinessId,
