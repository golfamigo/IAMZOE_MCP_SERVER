...
const ajv_1 = __importDefault(require("../utils/ajv")); // Import centralized AJV instance
...
// 驗證函式
const validateCreateBooking = ajv_1.default.compile(createBookingSchema);
// 建立預約 API
router.post('/bookings', auth_1.authenticateApiKey, async (req, res) => {
    ...
    try {
        // 檢查 business_id 是否存在
        const businessResult = await db_1.neo4jClient.runQuery('MATCH (b:Business {business_id: $business_id}) RETURN b', { business_id: bookingData.business_id });
        ...
        const serviceResult = await db_1.neo4jClient.runQuery('MATCH (bi:BookableItem {bookable_item_id: $bookable_item_id}) RETURN bi', { bookable_item_id: bookingData.bookable_item_id });
        ...
        // 檢查是否有時段衝突
        const conflictResult = await db_1.neo4jClient.runQuery(`MATCH (b:Booking {business_id: $business_id})
       ...
       RETURN b`, {
        ...
        // 創建新的預約
        const createResult = await db_1.neo4jClient.runQuery(`CREATE (b:Booking {
...
// 查詢可用時間 API
router.get('/bookable_items/:bookable_item_id/available_slots', auth_1.authenticateApiKey, async (req, res) => {
    ...
    try {
        ...
        // 查詢已有的預約
        const bookingsResult = await db_1.neo4jClient.runQuery(`MATCH (b:Booking {business_id: $business_id})
       ...
       ORDER BY b.booking_start_datetime`, {
        ...
        // 獲取商家的營業時間
        const hoursResult = await db_1.neo4jClient.runQuery(`MATCH (b:Business {business_id: $business_id}) 
       // 這裡應該有獲取營業時間的邏輯
...
// 取消預約 API
router.delete('/bookings/:booking_id', auth_1.authenticateApiKey, async (req, res) => {
