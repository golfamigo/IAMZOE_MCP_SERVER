...
// 驗證函式
const validateCreateStaff = ajv_1.default.compile(createStaffSchema);
// 建立員工 API
router.post('/staff', auth_1.authenticateApiKey, async (req, res) => {
    ...
    try {
        // 檢查 business_id 是否存在
        const businessResult = await db_1.neo4jClient.runQuery('MATCH (b:Business {business_id: $business_id}) RETURN b', { business_id: staffData.business_id });
        ...
        // 檢查 email 是否已存在
        const emailResult = await db_1.neo4jClient.runQuery('MATCH (s:Staff {staff_member_email: $staff_member_email, business_id: $business_id}) RETURN s', {
        ...
        // 創建 Staff 節點並建立關係
        const createResult = await db_1.neo4jClient.runQuery(`CREATE (s:Staff {
      ...
      RETURN s`, {
        ...
        // 回傳成功結果
        res.status(201).json({ staff_member_id });
        return;
    }
    catch (error) {
...
// 取得員工資訊 API
router.get('/staff/:staff_member_id', auth_1.authenticateApiKey, async (req, res) => {
...
// 更新員工資訊 API
router.put('/staff/:staff_member_id', auth_1.authenticateApiKey, async (req, res) => {
    const { staff_member_id } = req.params;
    ...
    try {
        ...
        if (updateData.staff_member_name || updateData.staff_member_phone) {
            ...
            await db_1.neo4jClient.runQuery(`MATCH (s:Staff {staff_member_id: $staff_member_id})-[:BELONGS_TO]->(u:User)
...
// 刪除員工 API
router.delete('/staff/:staff_member_id', auth_1.authenticateApiKey, async (req, res) => {
    const { staff_member_id } = req.params;
    try {
        // 查詢員工是否存在
        const staffResult = await db_1.neo4jClient.runQuery('MATCH (s:Staff {staff_member_id: $staff_member_id}) RETURN s', { staff_member_id });
        ...
        // 檢查員工是否有未完成的預約
        const bookingResult = await db_1.neo4jClient.runQuery(`MATCH (s:Staff {staff_member_id: $staff_member_id})<-[:ASSIGNED_TO]-(b:Booking)
       WHERE b.booking_status_code IN ['pending', 'confirmed']
       RETURN b`, { staff_member_id });
...
// 查詢員工列表 API
router.get('/staff', auth_1.authenticateApiKey, async (req, res) => {
...
// 尋找協作員工 API
router.get('/businesses/:business_id/staff/collaborators', auth_1.authenticateApiKey, async (req, res) => {
