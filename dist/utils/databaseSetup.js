...
exports.setupDatabaseConstraints = setupDatabaseConstraints;
exports.verifyDatabaseConnection = verifyDatabaseConnection;
exports.initializeDatabase = initializeDatabase;
const db_1 = require("../db");
const neo4jUtils_1 = require("./neo4jUtils");
/**
...
async function setupDatabaseConstraints() {
    try {
        ...
        for (const constraint of uniqueConstraints) {
            await db_1.neo4jClient.runQuery(constraint);
        }
        // 重要索引 - 索引語法保持不變
        const indexes = [
            // 現有索引
        ...
        for (const index of indexes) {
            await db_1.neo4jClient.runQuery(index);
        }
        // 關係索引 - 使用社區版兼容的語法
        try {
            // 使用替代語法，為關係屬性創建索引
...
async function verifyDatabaseConnection() {
    try {
        const result = await db_1.neo4jClient.runQuery('RETURN 1 as n');
        return result.records.length > 0 && (0, neo4jUtils_1.toJsNumber)(result.records[0].get('n')) === 1;
    }
    catch (error) {
