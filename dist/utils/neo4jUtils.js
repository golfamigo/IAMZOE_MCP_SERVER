...
exports.parseNeo4jDateTime = parseNeo4jDateTime;
 ...
 */
function toJsNumber(neo4jValue) {
    if (!neo4jValue)
        return 0;
    // 處理 Neo4j 整數類型 (帶有 low/high 屬性)
 ...
 */
function toISODateTimeString(dateTime) {
 ...
 */
function parseNeo4jDateTime(neo4jDateTime) {
    if (!neo4jDateTime)
        return new Date();
 ...
 * @returns 格式化後的日期字符串
 */
function formatLocalDate(date, locale = 'zh-TW') {
