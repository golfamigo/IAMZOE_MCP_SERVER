...
exports.scanToolDefinitions = scanToolDefinitions;
exports.checkToolCoverage = checkToolCoverage;
exports.generateToolDefinitionsTemplate = generateToolDefinitionsTemplate;
/**
 * 工具掃描器
 * 自動掃描指定目錄，收集所有工具定義
 */
 ...
 * @returns 收集到的工具定義陣列
 */
async function scanToolDefinitions(toolsDir) {
    try {
        ...
        for (const file of files) {
            ...
            if (file.isFile() && file.name.endsWith('.ts') && !excludeFiles.includes(file.name)) {
                ...
                try {
                    ...
                    // 嘗試匯入工具檔案
                    const toolModule = await Promise.resolve(`${toolFilePath}`).then(s => __importStar(require(s)));
                    // 搜尋檔案中所有可能的工具定義
                    // (大多數工具檔案會匯出單一工具或多個工具)
                    const potentialTools = Object.values(toolModule).filter(value => value &&
                        typeof value === 'object' &&
                        'name' in value &&
 ...
 */
async function checkToolCoverage(toolsDir, definedTools) {
    try {
 ...
 * @param toolsDir 工具目錄路徑
 */
async function generateToolDefinitionsTemplate(outputFile, toolsDir) {
    try {
